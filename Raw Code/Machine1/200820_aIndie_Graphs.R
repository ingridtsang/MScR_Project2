...
#aBDL2 <- readRDS(paste0(BDL2origin.path, Load.date, "_BDL2_annotated.RDS"))
...
BDL2_cellHM <- DoHeatmap(aBDL2_cleaned, group.by = "Cell.type", features = BDL2CLEANED_top3$gene, assay = "RNA", slot = "data", label = T, size = 3, angle = 90) + NoLegend()
...
mA5_cellHM <- DoHeatmap(mA5_MP, features = mA5CLEANED_top3$gene, size = 3, angle = 90) + NoLegend()
...
Bleo2_cellHM <- DoHeatmap(Bleo2_MP,  features = Bleo2CLEANED_top3$gene, size = 3, angle = 90) + NoLegend()
...
BDL2_cellHM <- DoHeatmap(BDL2_MP,  features = BDL2CLEANED_top3$gene, slot = "data", assay = "RNA", size = 3, angle = 90) + NoLegend()
ggsave(filename = paste0(BDL2Output.path, date.1, "_BDL2CLEANED_Fig_MPHM.png"), plot = BDL2_cellHM, width = 45, units = "cm")
...
CCl4_cellHM <- DoHeatmap(CCl4_MP, features = CCl4CLEANED_top3$gene, slot = "data", assay = "RNA", size = 3, angle = 90) + NoLegend()
